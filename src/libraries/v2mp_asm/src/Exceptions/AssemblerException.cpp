...
namespace V2MPAsm
{
	AssemblerException::AssemblerException(PublicErrorID errorID, const std::string& file, size_t line, size_t column) :
		std::exception(),
		m_Exception(errorID, file, line, column)
	{
	}

	AssemblerException::AssemblerException(PublicWarningID warningID, const std::string& file, size_t line, size_t column) :
		std::exception(),
